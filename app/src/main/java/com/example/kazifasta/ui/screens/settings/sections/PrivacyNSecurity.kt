...
@Composable
fun PrivacyNSecurity(onBackPressed: () -> Unit){
    Scaffold (
        containerColor = dark,
        topBar = { AppBar(onClick = { onBackPressed.invoke() }, title = stringResource(id = R.string.title_privacy_n_security)) },
        content = {innerPadding ->
            Column(modifier = Modifier.fillMaxSize().padding(innerPadding), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
                Text(text = stringResource(id = R.string.title_privacy_n_security))
            }
        }
    )
}